...
ArktsError ResetMemory(int32_t slotId, int32_t options, int32_t &resultCode)
{
    ...
    std::unique_lock<ffrt::mutex> callbackLock(profileContext->callbackMutex);
    ...
    const DownloadConfigurationAni &configAni, DownloadProfileResultAni &resultAni)
{
    ...
    return ConvertArktsErrorWithPermission(errorCode, "downloadProfile", Permission::GET_TELEPHONY_ESIM_STATE);
}
