...
static napi_value GetRadioTechSync(napi_env env, napi_callback_info info)
{
    ...
    if (IsValidSlotIdEx(slotId)) {
        int32_t psResult = DelayedRefSingleton<CoreServiceClient>::GetInstance().GetPsRadioTech(slotId,
            psRadioTechPre);
        int32_t csResult = DelayedRefSingleton<CoreServiceClient>::GetInstance().GetCsRadioTech(slotId,
            csRadioTechPre);
        if (psResult == TELEPHONY_SUCCESS && csResult == TELEPHONY_SUCCESS) {
...
static void NativeSetNetworkCapability(napi_env env, void *data)
{
    auto asyncContext = static_cast<NetworkCapabilityContext *>(data);
    if (!IsValidSlotId(asyncContext->slotId)) {
        TELEPHONY_LOGE("NativeSetNetworkCapability slotId is invalid");
        asyncContext->errorCode = ERROR_SLOT_ID_INVALID;
        return;
    }
...
static void SetNetworkCapabilityCallback(napi_env env, napi_status status, void *data)
{
    auto context = static_cast<NetworkCapabilityContext *>(data);
    napi_value callbackValue = nullptr;
    if (context->resolved) {
        napi_get_undefined(env, &callbackValue);
    } else {
...
static bool MatchSetNetworkCapabilityParameter(napi_env env, napi_value parameters[], size_t parameterCount)
{
    switch (parameterCount) {
        case PARAMETER_COUNT_THREE: {
            return NapiUtil::MatchParameters(env, parameters, { napi_number, napi_number, napi_number });
        }
        case PARAMETER_COUNT_FOUR: {
            return NapiUtil::MatchParameters(env, parameters, { napi_number, napi_number, napi_number, napi_function });
        }
        default:
            return false;
    }
...
static napi_value SetNetworkCapability(napi_env env, napi_callback_info info)
{
    ...
    void *data = nullptr;
    napi_get_cb_info(env, info, &parameterCount, parameters, &thisVar, &data);
    if (!MatchSetNetworkCapabilityParameter(env, parameters, parameterCount)) {
        TELEPHONY_LOGE("SetNetworkCapability parameter matching failed.");
        NapiUtil::ThrowParameterError(env);
        return nullptr;
    }
    auto asyncContext = std::make_unique<NetworkCapabilityContext>();
    ...
    return NapiUtil::HandleAsyncWork(
        env, asyncContext.release(), "SetNetworkCapability", NativeSetNetworkCapability, SetNetworkCapabilityCallback);
}

static void NativeGetNetworkCapability(napi_env env, void *data)
{
    auto asyncContext = static_cast<NetworkCapabilityContext *>(data);
    if (!IsValidSlotId(asyncContext->slotId)) {
        TELEPHONY_LOGE("NativeGetNetworkCapability slotId is invalid");
        asyncContext->errorCode = ERROR_SLOT_ID_INVALID;
        return;
    }
    if (!IsValidNetworkCapabilityType(asyncContext->networkCapabilityType)) {
...
static void GetNetworkCapabilityCallback(napi_env env, napi_status status, void *data)
{
    auto asyncContext = static_cast<NetworkCapabilityContext *>(data);
    napi_value callbackValue = nullptr;
    if (asyncContext->resolved) {
...
static bool MatchGetNetworkCapabilityParameter(napi_env env, napi_value parameters[], size_t parameterCount)
{
    switch (parameterCount) {
        case PARAMETER_COUNT_TWO: {
            return NapiUtil::MatchParameters(env, parameters, { napi_number, napi_number });
...
static napi_value GetNetworkCapability(napi_env env, napi_callback_info info)
{
    size_t parameterCount = PARAMETER_COUNT_THREE;
    napi_value parameters[PARAMETER_COUNT_THREE] = { 0 };
    napi_value thisVar = nullptr;
    void *data = nullptr;
    napi_get_cb_info(env, info, &parameterCount, parameters, &thisVar, &data);
    if (!MatchGetNetworkCapabilityParameter(env, parameters, parameterCount)) {
        TELEPHONY_LOGE("GetNetworkCapability parameter matching failed.");
        NapiUtil::ThrowParameterError(env);
        return nullptr;
    }
    auto asyncContext = std::make_unique<NetworkCapabilityContext>();
    ...
    return NapiUtil::HandleAsyncWork(
        env, asyncContext.release(), "GetNetworkCapability", NativeGetNetworkCapability, GetNetworkCapabilityCallback);
}
...
static napi_value IsNrSupported(napi_env env, napi_callback_info info)
{
    ...
    NAPI_CALL(env, napi_get_cb_info(env, info, &parameterCount, parameters, &thisVar, &data));
    if (!MatchIsNrSupportedParameter(env, parameters, parameterCount)) {
        TELEPHONY_LOGE("IsNrSupported parameter matching failed.");
        NapiUtil::ThrowParameterError(env);
        return nullptr;
    }
...
static void NativeIsManualNetworkScanning(napi_env env, void *data)
{
    ...
    auto callback = std::make_unique<ManualNetworkScanCallback>();
    callback->SetScanContext(asyncContext);
...
{
    auto callback = std::make_unique<ManualNetworkScanCallback>();
    if (!callback->CreateNapiCbRef(env, thisVar, argv)) {
        return false;
    }
    int32_t rpcResult = DelayedRefSingleton<CoreServiceClient>::GetInstance()
        .StartManualNetworkScanCallback(slotId, callback.release());
    if (rpcResult != TELEPHONY_SUCCESS) {
        ReportFunctionFailed(env, rpcResult, "startManualNetworkScan");
        return false;
    }
    return true;
}
...
static napi_value StopManualNetworkScan(napi_env env, napi_callback_info info)
{
    ...
    int32_t rpcResult = DelayedRefSingleton<CoreServiceClient>::GetInstance()
        .StopManualNetworkScanCallback(slotId);
    if (rpcResult != TELEPHONY_SUCCESS) {
        ReportFunctionFailed(env, rpcResult, "stopManualNetworkScan");
        return nullptr;
    }
    napi_value result = nullptr;
    napi_get_undefined(env, &result);
...
{
    napi_property_descriptor desc[] = {
        DECLARE_NAPI_WRITABLE_FUNCTION("getRadioTech", GetRadioTech),
        DECLARE_NAPI_WRITABLE_FUNCTION("getRadioTechSync", GetRadioTechSync),
        DECLARE_NAPI_WRITABLE_FUNCTION("getSignalInformation", GetSignalInfoList),
        DECLARE_NAPI_WRITABLE_FUNCTION("getSignalInformationSync", GetSignalInfoListSync),
        DECLARE_NAPI_WRITABLE_FUNCTION("getNetworkState", GetNetworkState),
        DECLARE_NAPI_WRITABLE_FUNCTION("setNetworkSelectionMode", SetNetworkSelectionMode),
        DECLARE_NAPI_WRITABLE_FUNCTION("getNetworkSelectionMode", GetNetworkSelectionMode),
        ...
        DECLARE_NAPI_WRITABLE_FUNCTION("getOperatorName", GetOperatorName),
        DECLARE_NAPI_WRITABLE_FUNCTION("getOperatorNameSync", GetOperatorNameSync),
        DECLARE_NAPI_WRITABLE_FUNCTION("setPreferredNetwork", SetPreferredNetwork),
        DECLARE_NAPI_WRITABLE_FUNCTION("getPreferredNetwork", GetPreferredNetwork),
        DECLARE_NAPI_WRITABLE_FUNCTION("setNetworkCapability", SetNetworkCapability),
        DECLARE_NAPI_WRITABLE_FUNCTION("getNetworkCapability", GetNetworkCapability),
        DECLARE_NAPI_WRITABLE_FUNCTION("getIMEI", GetIMEI),
        DECLARE_NAPI_WRITABLE_FUNCTION("getIMEISV", GetIMEISV),
        DECLARE_NAPI_WRITABLE_FUNCTION("getMEID", GetMEID),
        DECLARE_NAPI_WRITABLE_FUNCTION("sendUpdateCellLocationRequest", SendUpdateCellLocationRequest),
        DECLARE_NAPI_WRITABLE_FUNCTION("getCellInformation", GetCellInformation),
        DECLARE_NAPI_WRITABLE_FUNCTION("getPrimarySlotId", GetPrimarySlotId),
        DECLARE_NAPI_WRITABLE_FUNCTION("getUniqueDeviceId", GetUniqueDeviceId),
        DECLARE_NAPI_WRITABLE_FUNCTION("getNrOptionMode", GetNrOptionMode),
        DECLARE_NAPI_WRITABLE_FUNCTION("isNrSupported", IsNrSupported),
        DECLARE_NAPI_WRITABLE_FUNCTION("isNRSupported", IsNrSupported),
        DECLARE_NAPI_WRITABLE_FUNCTION("setPrimarySlotId", SetPrimarySlotId),
        DECLARE_NAPI_WRITABLE_FUNCTION("getImsRegInfo", GetImsRegInfo),
        DECLARE_NAPI_WRITABLE_FUNCTION("getBasebandVersion", GetBasebandVersion),
        DECLARE_NAPI_WRITABLE_FUNCTION("setNROptionMode", SetNrOptionMode),
        DECLARE_NAPI_WRITABLE_FUNCTION("getNROptionMode", GetNrOptionMode),
