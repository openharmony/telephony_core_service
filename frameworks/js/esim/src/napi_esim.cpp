...
void NativeGetSupportedPkids(napi_env env, void *data)
{
    ...
    auto asyncContext = static_cast<AsyncContext<std::string> *>(data);
    if (!IsValidSlotId(asyncContext->slotId)) {
    ...
    std::unique_ptr<GetSupportedPkidsResultCallback> callback =
        std::make_unique<GetSupportedPkidsResultCallback>(asyncContext);
    ...
    if (errorCode == TELEPHONY_SUCCESS) {
        asyncContext->cv.wait_for(callbackLock, std::chrono::seconds(WAIT_TIME_SECOND),
            [asyncContext] { return asyncContext->isCallbackEnd; });
    }
}
...
void NativeGetContractInfo(napi_env env, void *data)
{
    ...
    contractRequestData.publicKey = NapiUtil::ToUtf16(contractInfoCtx->contractRequestData.publicKey.data());
    contractRequestData.nonce = NapiUtil::ToUtf16(contractInfoCtx->contractRequestData.nonce.data());
    contractRequestData.pkid = NapiUtil::ToUtf16(contractInfoCtx->contractRequestData.pkid.data());
...
{
    napi_property_descriptor desc[] = {
        DECLARE_NAPI_WRITABLE_FUNCTION("isSupported", IsSupported),
        DECLARE_NAPI_WRITABLE_FUNCTION("addProfile", AddProfile),
        DECLARE_NAPI_WRITABLE_FUNCTION("getEid", GetEid),
        DECLARE_NAPI_WRITABLE_FUNCTION("getOsuStatus", GetOsuStatus),
        DECLARE_NAPI_WRITABLE_FUNCTION("startOsu", StartOsu),
        DECLARE_NAPI_WRITABLE_FUNCTION("getDownloadableProfileMetadata", GetDownloadableProfileMetadata),
        DECLARE_NAPI_WRITABLE_FUNCTION("getDownloadableProfiles", GetDownloadableProfiles),
        DECLARE_NAPI_WRITABLE_FUNCTION("downloadProfile", DownloadProfile),
        DECLARE_NAPI_WRITABLE_FUNCTION("getEuiccProfileInfoList", GetEuiccProfileInfoList),
        DECLARE_NAPI_WRITABLE_FUNCTION("getEuiccInfo", GetEuiccInfo),
        DECLARE_NAPI_WRITABLE_FUNCTION("deleteProfile", DeleteProfile),
        DECLARE_NAPI_WRITABLE_FUNCTION("switchToProfile", SwitchToProfile),
        DECLARE_NAPI_WRITABLE_FUNCTION("setProfileNickname", SetProfileNickname),
        DECLARE_NAPI_WRITABLE_FUNCTION("resetMemory", ResetMemory),
        DECLARE_NAPI_WRITABLE_FUNCTION("reserveProfilesForFactoryRestore", ReserveProfilesForFactoryRestore),
        DECLARE_NAPI_WRITABLE_FUNCTION("setDefaultSmdpAddress", SetDefaultSmdpAddress),
        DECLARE_NAPI_WRITABLE_FUNCTION("getDefaultSmdpAddress", GetDefaultSmdpAddress),
