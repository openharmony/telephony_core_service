...
bool RadioNetworkManager::GetPreferredNetwork(int32_t slotId, const sptr<INetworkSearchCallback> &callback)
{
    return DelayedSingleton<NetworkSearchServiceProxyHolder>::GetInstance()->GetPreferredNetwork(slotId, callback);
}

bool RadioNetworkManager::SetPreferredNetwork(
    int32_t slotId, int32_t networkMode, const sptr<INetworkSearchCallback> &callback)
{
    return DelayedSingleton<NetworkSearchServiceProxyHolder>::GetInstance()->SetPreferredNetwork(
        slotId, networkMode, callback);
}

bool RadioNetworkManager::SendUpdateCellLocationRequest()
{
    return DelayedSingleton<NetworkSearchServiceProxyHolder>::GetInstance()->SendUpdateCellLocationRequest();
}

std::vector<sptr<CellInformation>> RadioNetworkManager::GetCellInfoList(int32_t slotId)
{
    return DelayedSingleton<NetworkSearchServiceProxyHolder>::GetInstance()->GetCellInfoList(slotId);
}
