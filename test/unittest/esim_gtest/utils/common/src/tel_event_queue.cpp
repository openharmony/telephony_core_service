...
constexpr int PRINT_INTELVAL_MINUTES = 5;
...
{
    if (currentQueueEvents_ <= 0) {
        currentQueueEvents_ = 1;
    }
    submitedToFFRTEvents_++;
    currentQueueEvents_--;
}
...
void TelEventQueue::EventStats::PrintEventStats(std::string &name)
{
    ...
    lastPrintTime_ = now;
    TELEPHONY_LOGI(
        "%{public}s, totalHandledEvents %{public}llu, currentQueueEvents %{public}llu,"
        " submitedToFFRTEvents %{public}llu, executedEvents %{public}llu, removedEvents %{public}llu",
        name.c_str(),
        static_cast<unsigned long long>(totalHandledEvents_.load()),
        static_cast<unsigned long long>(currentQueueEvents_.load()),
        static_cast<unsigned long long>(submitedToFFRTEvents_.load()),
        static_cast<unsigned long long>(executedEvents_.load()),
        static_cast<unsigned long long>(removedEvents_.load())
    );
}
