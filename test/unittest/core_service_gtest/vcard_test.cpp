...
/**
 * @tc.number   Telephony_VcardTest_103
 * @tc.name     test simple vcard
 * @tc.desc     Function test
 */
HWTEST_F(VcardTest, Telephony_VCardTest_103, Function | MediumTest | Level2)
{
    AccessToken token;
    std::shared_ptr<DataShare::DataShareHelper> dataShareHelper =
        CreateDataShareHelper(TELEPHONY_CORE_SERVICE_SYS_ABILITY_ID, CONTACT_URI);
    if (dataShareHelper != nullptr) {
        ...
        resultSetAfter->GetRowCount(rowCountAfter);
        TELEPHONY_LOGE("VCardTest QueryContact rowCountAfter= %{public}d", rowCountAfter);
        EXPECT_EQ(((rowCountAfter - rowCountBefor) == 1), true);
        OHOS::DataShare::DataSharePredicates predicates2;
        predicates2.EqualTo(ContactData::TYPE_ID, "13")
            ->And()
            ->EqualTo(ContactData::DETAIL_INFO, "20230102")
            ->And()
            ->EqualTo(ContactData::LABEL_NAME, "test")
            ->And()
            ->EqualTo(ContactData::LABEL_ID, "1");
        auto resultSetQueryContactData = VCardRdbHelper::GetInstance().QueryContactData(columns, predicates2);
        int rowCount = 0;
        resultSetQueryContactData->GetRowCount(rowCount);
...
/**
 * @tc.number   Telephony_VcardTest_104
 * @tc.name     test simple vcard
 * @tc.desc     Function test
 */
HWTEST_F(VcardTest, Telephony_VCardTest_104, Function | MediumTest | Level2)
{
    AccessToken token;
    std::shared_ptr<DataShare::DataShareHelper> dataShareHelper =
        CreateDataShareHelper(TELEPHONY_CORE_SERVICE_SYS_ABILITY_ID, CONTACT_URI);
    if (dataShareHelper != nullptr) {
        ...
        EXPECT_EQ(((rowCountAfter - rowCountBefor) == 1), true);
        OHOS::DataShare::DataSharePredicates predicates2;
        predicates2.EqualTo(ContactData::TYPE_ID, "11")
            ->And()
            ->EqualTo(ContactData::DETAIL_INFO, "realationName")
            ->And()
            ->EqualTo(ContactData::LABEL_NAME, "labelName")
            ->And()
            ->EqualTo(ContactData::LABEL_ID, "labelId");
        auto resultSetQueryContactData = VCardRdbHelper::GetInstance().QueryContactData(columns, predicates2);
        int rowCount = 0;
        resultSetQueryContactData->GetRowCount(rowCount);
        EXPECT_EQ(rowCount, 1);
...
/**
 * @tc.number   Telephony_VcardTest_3.0
 * @tc.name     test simple vcard
 * @tc.desc     Function test
 */
...
END:VCARD
    ...
    std::string expvalues = R"(
 ...
 IYy5Y+dnHatAsfsAHfArmS1mPoh1gT8x9qtk1rQX7tCe5DIapzGtGBQm71SlqGWjnIH6Vowt
 zmhAy/E3vV6F6tEMuxlWIyAfrVxCCAO1VZEEyYA4AApxNGwyJ+lVJRUsaKMw61SlFQzRAP/Z)";
    EXPECT_EQ(contacts[0]->GetPhotos()[0]->GetBytes(), VCardUtils::DecodeBase64(expvalues));
}
