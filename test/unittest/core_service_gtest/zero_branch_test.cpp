...
HWTEST_F(BranchTest, Telephony_SimSmsController_001, Function | MediumTest | Level1)
{
    ...
    std::shared_ptr<Telephony::SimFileManager> simFileManager =
        std::make_shared<SimFileManager>(telRilManager, simStateManager);
...
HWTEST_F(BranchTest, Telephony_MultiSimController_001, Function | MediumTest | Level1)
{
    ...
    AAFwk::Want want;
    EXPECT_TRUE(multiSimController->PublishSimFileEvent(want, 0, ""));
...
HWTEST_F(BranchTest, Telephony_SimManager_003, Function | MediumTest | Level1)
{
    ...
    EXPECT_NE(simManager->SetSimLabelIndexByIccId("01234567890123456789", 0), TELEPHONY_ERR_SUCCESS);
...
HWTEST_F(BranchTest, Telephony_SIMUtils_001, Function | MediumTest | Level1)
{
    ...
    int recordLen = 0;
    std::shared_ptr<unsigned char> data = simUtils->HexStringConvertToBytes(str, recordLen);
    unsigned char *tlv = data.get();
    simUtils->Gsm7bitConvertToString(tlv + 3, 7, tlv[2]);
 
    str = "4308805066D309AA1D01FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF";
    recordLen = 0;
    std::shared_ptr<unsigned char> data1 = simUtils->HexStringConvertToBytes(str, recordLen);
    unsigned char *tlv1 = data1.get();
    simUtils->Gsm7bitConvertToString(tlv1 + 3, 7, tlv1[2]);
...
HWTEST_F(BranchTest, Telephony_MultiSimMonitor_002, Function | MediumTest | Level1)
{
    ...
    ASSERT_TRUE(simFileManagerWeak.size() != 0);
}
...
HWTEST_F(BranchTest, Telephony_MultiSimMonitor_006, Function | MediumTest | Level1)
{
    ...
    int32_t userId = 100;
    multiSimMonitor->isDataShareReady_ = true;
    multiSimMonitor->OnUserSwitched(userId);
    EXPECT_EQ(multiSimMonitor->lastUserId_, 100);
    userId = 101;
    multiSimMonitor->isDataShareReady_ = true;
    multiSimMonitor->OnUserSwitched(userId);
    EXPECT_EQ(multiSimMonitor->lastUserId_, 101);
    userId = 100;
    ...
    multiSimMonitor->initRebootDetectRemainCount_ = 1;
...
/**
 * @tc.number   Telephony_MultiSimMonitor_OnDataShareReady_isUserSwitch
 * @tc.name     test error branch
 * @tc.desc     Function test
 */
HWTEST_F(BranchTest, Telephony_MultiSimMonitor_OnDataShareReady_isUserSwitch, Function | MediumTest | Level1)
{
    std::shared_ptr<TelRilManager> telRilManager = std::make_shared<TelRilManager>();
    auto simStateManagerPtr = std::make_shared<SimStateManager>(telRilManager);
    auto telRilManagerWeak = std::weak_ptr<TelRilManager>(telRilManager);
...
/**
 * @tc.number   Telephony_MultiSimMonitor_UpdateAllSimData
 * @tc.name     test error branch
 * @tc.desc     Function test
 */
HWTEST_F(BranchTest, Telephony_MultiSimMonitor_UpdateAllSimData, Function | MediumTest | Level1)
{
    ...
    multiSimMonitor->isSimStateChange_ = true;
    multiSimMonitor->UpdateAllSimData(userId);
    EXPECT_FALSE(multiSimMonitor->isSimStateChange_);
    multiSimMonitor->isSimStateChange_ = true;
    multiSimMonitor->UpdateAllSimData(101);
    EXPECT_FALSE(multiSimMonitor->isSimStateChange_);
    multiSimMonitor->isSimStateChange_ = true;
    multiSimMonitor->UpdateAllSimData(101);
    EXPECT_TRUE(multiSimMonitor->isSimStateChange_);
}
