...
{
    SecurityToken token;
    auto result = DelayedSingleton<CoreService>::GetInstance()->GetImei(0, nullptr);
    ASSERT_EQ(result, TELEPHONY_ERR_ILLEGAL_USE_OF_SYSTEM_API);
}
...
HWTEST_F(CoreServiceTest, CoreService_GetImeiSv_001, Function | MediumTest | Level1)
{
    ...
    auto result = DelayedSingleton<CoreService>::GetInstance()->GetImeiSv(0, nullptr);
    ASSERT_EQ(result, TELEPHONY_ERR_ILLEGAL_USE_OF_SYSTEM_API);
}
