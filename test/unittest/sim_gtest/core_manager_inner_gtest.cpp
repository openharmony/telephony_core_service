...
HWTEST_F(CoreManagerInnerTest, IsEsim_001, Function | MediumTest | Level1)
{
    mInner.simManager_ = nullptr;
    int32_t slotId = 0;
    int32_t ret = mInner.IsEsim_001(slotId);
    EXPECT_FALSE(ret);

    mInner.simManager_ = mockeSimManager;
    EXPECT_CALL(*mockeSimManager, IsEsim(_)).WillOnce(
        Return(true));
    ret = mInner.IsEsim(slotId);
    EXPECT_TRUE(ret);
}

HWTEST_F(CoreManagerInnerTest, ClearSimLabel_001, Function | MediumTest | Level1)
{
    mInner.simManager_ = nullptr;
    int32_t ret = mInner.ClearSimLabel(slotId);
    EXPECT_EQ(ret, TELEPHONY_ERR_LOCAL_PTR_NULL);

    mInner.simManager_ = mockeSimManager;
    EXPECT_CALL(*mockeSimManager, ClearSimLabel(_)).WillOnce(
        Return(TELEPHONY_ERR_SUCCESS));
    ret = mInner.ClearSimLabel(SimType::PSIM);
    EXPECT_EQ(ret, TELEPHONY_ERR_SUCCESS);
}

HWTEST_F(CoreManagerInnerTest, UpdateSimPresent_001, Function | MediumTest | Level1)
{
    mInner.simManager_ = nullptr;
    bool isShowPresent = true;
    int32_t ret = mInner.UpdateSimPresent(0, isShowPresent);
    EXPECT_EQ(ret, TELEPHONY_ERR_LOCAL_PTR_NULL);

    mInner.simManager_ = mockeSimManager;
    EXPECT_CALL(*mockeSimManager, UpdateSimPresent(_, _)).WillOnce(
        Return(TELEPHONY_ERR_SUCCESS));
    ret = mInner.UpdateSimPresent(1, isShowPresent);
    EXPECT_EQ(ret, TELEPHONY_ERR_SUCCESS);
}

HWTEST_F(CoreManagerInnerTest, UpdateEsimOpName_001, Function | MediumTest | Level1)
{
    mInner.simManager_ = nullptr;
    std::string opName = "中国移动";
    std::string iccId = "12345123451234512345";
    int32_t ret = mInner.UpdateEsimOpName(iccId, opName);
    EXPECT_EQ(ret, TELEPHONY_ERR_LOCAL_PTR_NULL);

    mInner.simManager_ = mockeSimManager;
    EXPECT_CALL(*mockeSimManager, UpdateEsimOpName(_, _)).WillOnce(
        Return(TELEPHONY_ERR_SUCCESS));
    ret = mInner.UpdateEsimOpName(iccId, opName);
    EXPECT_EQ(ret, TELEPHONY_ERR_SUCCESS);
    
    mInner.simManager_ = nullptr;
    mInner.CheckIfNeedSwitchMainSlotId(true);
    mInner.simManager_ = mockeSimManager;
    EXPECT_CALL(*mockeSimManager, CheckIfNeedSwitchMainSlotId(_)).WillOnce(
        Return());
    mInner.CheckIfNeedSwitchMainSlotId(false);
}

HWTEST_F(CoreManagerInnerTest, SetTargetPrimarySlotId_001, Function | MediumTest | Level1)
{
    mInner.simManager_ = nullptr;
    int32_t ret = mInner.SetTargetPrimarySlotId(true, 0);
    EXPECT_EQ(ret, TELEPHONY_ERR_LOCAL_PTR_NULL);

    mInner.simManager_ = mockSimManager;
    EXPECT_CALL(*mockSimManager, SetTargetPrimarySlotId(_, _)).WillRepeatedly(
        Return(TELEPHONY_ERR_SUCCESS));
    ret = mInner.SetTargetPrimarySlotId(true, 0);
