...
void GetNeighboringCellInfoList(const uint8_t *data, size_t size)
{
    if (!IsServiceInited()) {
        return;
    }
    ...
    dataMessageParcel.RewindRead(0);
    MessageParcel reply;
    DelayedSingleton<CoreService>::GetInstance()->OnGetNeighboringCellInfoList(dataMessageParcel, reply);
}
...
void FactoryReset(const uint8_t *data, size_t size)
{
    ...
    dataMessageParcel.RewindRead(0);
    MessageParcel reply;
    DelayedSingleton<CoreService>::GetInstance()->OnFactoryReset(dataMessageParcel, reply);
}

void IsAllowedInsertApn(const uint8_t *data, size_t size)
{
    if (!IsServiceInited()) {
        return;
    }
    ...
    dataMessageParcel.RewindRead(0);
    MessageParcel reply;
    DelayedSingleton<CoreService>::GetInstance()->OnIsAllowedInsertApn(dataMessageParcel, reply);
}

void GetOpkeyVersion(const uint8_t *data, size_t size)
{
    if (!IsServiceInited()) {
        return;
    }
    ...
    dataMessageParcel.RewindRead(0);
    MessageParcel reply;
    DelayedSingleton<CoreService>::GetInstance()->OnGetOpkeyVersion(dataMessageParcel, reply);
}

void GetOpnameVersion(const uint8_t *data, size_t size)
{
    if (!IsServiceInited()) {
        return;
    }
    ...
    dataMessageParcel.RewindRead(0);
    MessageParcel reply;
    DelayedSingleton<CoreService>::GetInstance()->OnGetOpnameVersion(dataMessageParcel, reply);
}
