...
void SimManagerFunc(const uint8_t *data, size_t size)
{
    ...
    std::vector<std::weak_ptr<Telephony::SimFileManager>> simFileManagers;
    auto multiSimMonitor = std::make_shared<MultiSimMonitor>(multiSimController, simStateManager, simFileManagers);
