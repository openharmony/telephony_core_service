...
void NetworkSearchManager::RegisterCoreNotify(int32_t slotId, HANDLE &handler, int32_t what)
{
    ...
    auto inner = FindManagerInner(slotId);
    if (inner != nullptr && inner->observerHandler_ != nullptr) {
...
{
    auto inner = FindManagerInner(slotId);
    if (inner != nullptr && inner->networkSearchResult_ != nullptr) {
...
int32_t NetworkSearchManager::SetPreferredNetwork(int32_t slotId, int32_t networkMode, NSCALLBACK &callback)
{
    if (simManager_ == nullptr || eventSender_ == nullptr) {
        TELEPHONY_LOGE("SetPreferredNetwork simManager_ or eventSender_ is nullptr");
        return TELEPHONY_ERR_LOCAL_PTR_NULL;
    }
...
int32_t NetworkSearchManager::GetImsRegStatus(int32_t slotId, ImsServiceType imsSrvType, ImsRegInfo &info)
{
    ...
    auto inner = FindManagerInner(slotId);
    if (inner == nullptr || inner->networkSearchState_ == nullptr) {
        TELEPHONY_LOGE("NetworkSearchManagerInner or networkSearchState is nullptr!");
        return TELEPHONY_ERR_LOCAL_PTR_NULL;
    }
...
int32_t NetworkSearchManager::GetImei(int32_t slotId, std::u16string &imei)
{
    ...
    if (IsSatelliteEnabled() && slotId == satelliteClient->GetSatelliteSlotId()) {
...
int32_t NetworkSearchManager::SetNrOptionMode(int32_t slotId, int32_t mode, NSCALLBACK &callback)
{
    ...
    int32_t errCode = TELEPHONY_ERR_SUCCESS;
    int32_t masterSlotId = -1;
    if (simManager_ != nullptr) {
        simManager_->GetPrimarySlotId(masterSlotId);
    }
    if (TELEPHONY_EXT_WRAPPER.setNrOptionModeExt_ != nullptr &&
        TELEPHONY_EXT_WRAPPER.setNrOptionModeExt_(slotId, masterSlotId, mode, errCode)) {
        return errCode;
    }
    if (eventSender_ == nullptr) {
        TELEPHONY_LOGE("slotId:%{public}d eventSender_ is null", slotId);
...
void NetworkSearchManager::TriggerSimRefresh(int32_t slotId)
{
    ...
    auto inner = FindManagerInner(slotId);
    if (inner != nullptr && simManager_ != nullptr && inner->networkSearchHandler_ != nullptr) {
...
int32_t NetworkSearchManager::StartOrStopManualNetworkScan(int32_t slotId, bool isStart)
{
    ...
    if (simState == SimState::SIM_STATE_NOT_PRESENT || simState == SimState::SIM_STATE_UNKNOWN) {
        ...
        if (isStart) {
            ...
            auto manualScanResult = std::make_shared<ManualScanResult>();
            manualScanResult->isFinished = true;
            auto event = AppExecFwk::InnerEvent::Get(RadioEvent::RADIO_MANUAL_SEARCHED_PLMN_LIST, manualScanResult);
            networkSearchHandler->SendEvent(event);
        }
    }
    return TELEPHONY_ERR_SUCCESS;
