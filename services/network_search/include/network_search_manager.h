...
public:
    ...
    inline void SetSkipUnsolRptFlag(int32_t slotId, bool res)
    {
        auto inner = FindManagerInner(slotId);
        if (inner != nullptr) {
            inner->SetSkipUnsolRptFlag(res);
        }
    }
    inline bool GetSkipUnsolRptFlag(int32_t slotId)
    {
        auto inner = FindManagerInner(slotId);
        if (inner != nullptr) {
            return inner->GetSkipUnsolRptFlag();
        }
        return false;
    }
