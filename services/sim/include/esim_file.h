...
constexpr static const int32_t WAIT_TIME_SHORT_SECOND_FOR_ESIM = 5;
constexpr static const int32_t WAIT_TIME_OPEN_CHANNEL_FOR_ESIM = 8;
constexpr static const int32_t WAIT_TIME_CLOSE_CHANNEL_FOR_ESIM = 4;
...
private:
    ...
    void SyncCloseChannel(const SimMessage msgType);
    bool IsLogicChannelOpen(const SimMessage msgType);
    void ProcessEsimOpenChannel(const std::u16string &aid, const SimMessage msgType);
    bool ProcessEsimOpenChannelDone(const AppExecFwk::InnerEvent::Pointer &event);
    void ProcessEsimCloseChannel(const SimMessage msgType);
    bool ProcessEsimCloseChannelDone(const AppExecFwk::InnerEvent::Pointer &event);
    ResultInnerCode ObtainChannelSuccessExclusive(const SimMessage msgType);
    void ProcessEvent(const AppExecFwk::InnerEvent::Pointer &event);
    ResultInnerCode ObtainChannelSuccessAllowSameAidReuse(const std::u16string &aid);
