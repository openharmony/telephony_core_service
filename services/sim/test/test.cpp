...
static bool TestQueryIccDiallingNumbers()
{
    ...
    std::cout << "please select type: 1.public dialling numbers 2.fix dialing number" << std::endl;
    std::cin >> testType;
...
static bool TestSetLockState()
{
    ...
    g_telephonyService->SetLockState(SLOT_ID, Str8ToStr16(pin.c_str()), mode, response);
...
static bool TestGetMaxSimCount()
{
    ...
    string expect = (result != INVALID_VALUE) ? "success" : "fail";
    std::cout << "TelephonyTestService Remote GetMaxSimCount result [" << result << "] " << expect
              << std::endl;
    return true;
}
...
static void InitFuncMap()
{
    ...
    g_funcMap[InputCmd::INPUT_CHECK_PIN] = TestGetLockState;
    g_funcMap[InputCmd::INPUT_ENABLE_PIN] = TestSetLockState;
