...
void SimFileParse::ParsePnn(const std::vector<std::string> &records, SimFile &simFile)
{
    ...
    for (const auto &dataPnn : records) {
        ...
        if (recordLen >= (tagAndLength + static_cast<int>(tlv[NETWORK_NAME_LENGTH])) &&
            ...
            file->longName =
                SIMUtils::Gsm7bitConvertToString(tlv + NETWORK_NAME_TEXT_STRING, tlv[NETWORK_NAME_LENGTH] - 1,
                    tlv[NETWORK_NAME_CODING_SCHEME]);
        }
        ...
        if (recordLen > (shortNameOffset + tagAndLength) &&
            ...
            file->shortName = SIMUtils::Gsm7bitConvertToString(tlv + (shortNameOffset + NETWORK_NAME_TEXT_STRING),
                tlv[shortNameOffset + NETWORK_NAME_LENGTH] - 1, tlv[shortNameOffset + NETWORK_NAME_CODING_SCHEME]);
        }
...
void SimFileParse::ParseOpl(const std::vector<std::string> &records, SimFile &simFile)
{
    ...
    for (const auto &dataOpl : records) {
        ...
        file->lacStart = static_cast<int>(strtol(
            dataOpl.substr(MCCMNC_LEN, HALF_BYTE_LEN).c_str(), nullptr, HEXADECIMAL));
        file->lacEnd = static_cast<int>(strtol(
            dataOpl.substr(MCCMNC_LEN + HALF_BYTE_LEN, HALF_BYTE_LEN).c_str(), nullptr, HEXADECIMAL));
        file->pnnRecordId = static_cast<int>(strtol(
            dataOpl.substr(MCCMNC_LEN + BYTE_LENGTH, HALF_LEN).c_str(), nullptr, HEXADECIMAL));
...
void SimFileParse::ParseOpl5g(const std::vector<std::string> &records, SimFile &simFile)
{
    ...
    for (const auto &dataOpl : records) {
        ...
        file->plmnNumeric = plmn;
        file->lacStart = static_cast<int>(strtol(
            dataOpl.substr(MCCMNC_LEN, LAC_RANGE_LEN).c_str(), nullptr, HEXADECIMAL));
        file->lacEnd = static_cast<int>(strtol(
            dataOpl.substr(MCCMNC_LEN + LAC_RANGE_LEN, LAC_RANGE_LEN).c_str(), nullptr, HEXADECIMAL));
        file->pnnRecordId = static_cast<int>(strtol(
            dataOpl.substr(MCCMNC_LEN + LAC_RANGE_LEN + LAC_RANGE_LEN, HALF_LEN).c_str(), nullptr, HEXADECIMAL));
