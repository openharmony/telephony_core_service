...
void SimManager::RegisterCoreNotify(int32_t slotId, const std::shared_ptr<AppExecFwk::EventHandler> &handler, int what)
{
    if ((what >= RadioEvent::RADIO_IMSI_LOADED_READY) && (what <= RadioEvent::RADIO_SIM_RECORDS_LOADED)) {
        std::shared_lock<ffrt::shared_mutex> lck(mtx_);
        if ((simFileManager_.empty() || !IsValidSlotId(slotId, simFileManager_)) || (simFileManager_[slotId] == nullptr)) {
        ...
        std::shared_lock<ffrt::shared_mutex> lck(mtx_);
        if ((simStateManager_.empty() || !IsValidSlotId(slotId, simStateManager_)) || (simStateManager_[slotId] == nullptr)) {
    ...
    int32_t fileId, const std::string &data, const std::string &path, SimAuthenticationResponse &response)
{
    ...
    requestInfo.p2 = static_cast<int>(strtol(
        data.substr(SIM_IO_DATA_P2_OFFSET, SIM_IO_DATA_STR_LEN).c_str(), nullptr, SIM_IO_HEX_SIGN));
    requestInfo.p3 = static_cast<int>(strtol(
        data.substr(SIM_IO_DATA_P3_OFFSET, SIM_IO_DATA_STR_LEN).c_str(), nullptr, SIM_IO_HEX_SIGN));
...
int32_t SimManager::GetEuiccProfileInfoList(int32_t slotId, GetEuiccProfileInfoListInnerResult &euiccProfileInfoList)
{
    if ((!IsValidSlotId(slotId, simFileManager_)) || (simFileManager_[slotId] == nullptr)) {
        TELEPHONY_LOGE("simFileManager is null!");
        return TELEPHONY_ERR_LOCAL_PTR_NULL;
...
int32_t SimManager::GetEuiccChallenge(int32_t slotId, int32_t portIndex, ResponseEsimInnerResult &responseResult)
{
    if ((!IsValidSlotId(slotId, simFileManager_)) || (simFileManager_[slotId] == nullptr)) {
        TELEPHONY_LOGE("simFileManager is null!");
        return TELEPHONY_ERR_LOCAL_PTR_NULL;
    ...
    CancelReason cancelReason, ResponseEsimInnerResult &responseResult)
{
    ...
    if (responseResult.resultCode_ != static_cast<int32_t>(ResultCode::RESULT_OK)) {
        return TELEPHONY_ERR_FAIL;
    }
    return TELEPHONY_ERR_SUCCESS;
}
...
int32_t SimManager::ResetMemory(int32_t slotId, ResetOption resetOption, int32_t &enumResult)
{
    if ((!IsValidSlotId(slotId, simFileManager_)) || (simFileManager_[slotId] == nullptr)) {
...
int32_t SimManager::DeleteProfile(int32_t slotId, const std::u16string &iccId, int32_t &enumResult)
{
    if ((!IsValidSlotId(slotId, simFileManager_)) || (simFileManager_[slotId] == nullptr)) {
        TELEPHONY_LOGE("simFileManager is null!");
        return TELEPHONY_ERR_LOCAL_PTR_NULL;
