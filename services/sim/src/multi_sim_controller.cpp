...
bool MultiSimController::ForgetAllData()
{
    ...
    TELEPHONY_LOGI("ForgetAllData start");
...
int32_t MultiSimController::SwapM0M2SimCards(int32_t slotId)
{
    return TELEPHONY_EXT_WRAPPER.SwapM0M2SimCards(slotId);
}

int32_t MultiSimController::NeedSwitchSimCard(int32_t slotId)
{
    if (!IsValidData(slotId)) {
        TELEPHONY_LOGE("no sim card");
        return TELEPHONY_ERR_NO_SIM_CARD;
    }
    slotId = SwapM0M2SimCards(slotId);
    return SetTargetPrimarySlotId(true, slotId);
}
...
int32_t MultiSimController::InsertData(int slotId, const std::string &newIccId)
{
    ...
    int simLabelIndex = PSIM1;
    if (isSupportEsimMep_) {
        simLabelIndex = multiSimHelper_->GetPsimLabelIndex(slotId);
    } else {
        if ((slotId == 0 && simLabelState == PSIM2_ESIM) || (slotId == 1 && simLabelState == PSIM1_PSIM2)) {
            simLabelIndex = PSIM2;
        }
    ...
    DataShare::DataShareValuesBucket values;
    SimDataBuilder(slotId, values, newIccId, simLabelIndex, IsEsim(slotId));
    int64_t id;
...
int32_t MultiSimController::InsertEsimData(const std::string &iccId, int32_t esimLabel, const std::string &operatorName)
{
    ...
    SimDataBuilder(INVALID_VALUE, values, iccId, esimLabel, true);
    int64_t id;
    std::unique_lock<ffrt::mutex> lock(writeDbMutex_);
...
void MultiSimController::SimDataBuilder(int slotId, DataShare::DataShareValuesBucket &values, const std::string &iccId,
...
void MultiSimController::UpdateSubState(int32_t slotId, int32_t enable)
{
    ...
    UpdateActiveSimInProgress(slotId, 0);
}
...
{
    UpdateActiveSimInProgress(slotId, 1);
...
int32_t MultiSimController::SetActiveSimSatellite(int32_t slotId, int32_t enable, bool force)
{
    ...
    UpdateActiveSimInProgress(slotId, ACTIVE_SIM_IN_PROGRESS);
    if (force) {
        UpdateSubState(slotId, enable);
        return TELEPHONY_ERR_SUCCESS;
    }
    if (simDbHelper_ == nullptr) {
        TELEPHONY_LOGE("failed by nullptr");
        UpdateActiveSimInProgress(slotId, ACTIVE_SIM_NOT_IN_PROGRESS);
        return TELEPHONY_ERR_LOCAL_PTR_NULL;
    }
    DataShare::DataShareValuesBucket values;
    DataShare::DataShareValueObject valueObj(enable);
...
int32_t MultiSimController::SetPrimarySlotId(int32_t slotId, bool isUserSet)
{
    ...
    if (slotId == SIM_SLOT_3) {
        return NeedSwitchSimCard(SIM_SLOT_3);
    }
...
{
    return multiSimHelper_->IsEsim(slotId);
}
...
void MultiSimController::UpdateActiveSimInProgress(int32_t slotId, int32_t enable)
{
    std::unique_lock<ffrt::mutex> lock(activeSimInProgressMutex_);
    if (slotId >= SIM_SLOT_0 && static_cast<uint32_t>(slotId) < isSetActiveSimInProgress_.size()) {
        isSetActiveSimInProgress_[slotId] = enable;
    }
}
} // namespace Telephony
} // namespace OHOS
