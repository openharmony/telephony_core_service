...
void CoreServiceStub::AddHandlerNetWorkToMap()
{
    ...
    memberFuncMap_[uint32_t(InterfaceID::GET_NETWORK_SEARCH_RESULT)] =
...
void CoreServiceStub::AddHandlerSimToMap()
{
    ...
    memberFuncMap_[uint32_t(InterfaceID::CHECK_PIN)] = &CoreServiceStub::OnGetLockState;
    memberFuncMap_[uint32_t(InterfaceID::SWITCH_PIN)] = &CoreServiceStub::OnSetLockState;
...
int32_t CoreServiceStub::OnGetNetworkSelectionMode(MessageParcel &data, MessageParcel &reply)
{
    ...
    sptr<IRemoteObject> remoteCallback = data.ReadRemoteObject();
    if (remoteCallback != nullptr) {
        TELEPHONY_LOGE("CoreServiceStub::OnGetNetworkSelectionMode remote callback is not null.");
        callback = iface_cast<INetworkSearchCallback>(remoteCallback);
    }
    bool result = false;
    if (callback != nullptr) {
...
int32_t CoreServiceStub::OnUnlockPin2(MessageParcel &data, MessageParcel &reply)
{
    ...
    TELEPHONY_LOGI("CoreServiceStub::OnUnlockPin2(), pin2 = %{public}s, slotId = %{public}d",
        Str16ToStr8(pin2).c_str(), slotId);
...
int32_t CoreServiceStub::OnAlterPin2(MessageParcel &data, MessageParcel &reply)
{
    ...
    TELEPHONY_LOGI(
        "CoreServiceStub::OnAlterPin2(), newPin2 = %{public}s, oldPin2 = %{public}s, slotId = %{public}d",
...
int32_t CoreServiceStub::OnSetLockState(MessageParcel &data, MessageParcel &reply)
{
    ...
    TELEPHONY_LOGI("CoreServiceStub::OnSetLockState(), pin = %{public}s, mode = %{public}d, slotId = %{public}d",
        Str16ToStr8(pin).c_str(), mode, slotId);
...
{
    int32_t slotId = data.ReadInt32();
    TELEPHONY_LOGI("CoreServiceStub::OnGetLockState(), slotId = %{public}d", slotId);
    int32_t result = GetLockState(slotId);
    bool ret = reply.WriteInt32(result);
    if (!ret) {
...
int32_t CoreServiceStub::OnGetPreferredNetwork(MessageParcel &data, MessageParcel &reply)
{
    ...
    sptr<IRemoteObject> remoteCallback = data.ReadRemoteObject();
    if (remoteCallback != nullptr) {
        TELEPHONY_LOGE("CoreServiceStub::OnGetPreferredNetwork remote callback is not null.");
        callback = iface_cast<INetworkSearchCallback>(remoteCallback);
    }
    bool result = false;
    if (callback != nullptr) {
...
int32_t CoreServiceStub::OnGetActiveSimAccountInfoList(MessageParcel &data, MessageParcel &reply)
{
    ...
    while (it != iccAccountInfoList.end()) {
        TELEPHONY_LOGI("OnGetActiveSimAccountInfoList slotindex = %{public}d, showname = %{public}s",
            (*it).slotIndex, Str16ToStr8((*it).showName).c_str());
...
{
    bool result = SendUpdateCellLocationRequest();
    bool ret = reply.WriteBool(result);
    if (!ret) {
