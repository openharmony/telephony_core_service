...
void TelRilNetwork::AddHandlerToMap()
{
    ...
    memberFuncMap_[HREQ_NETWORK_GET_IMEI] = &TelRilNetwork::GetImeiResponse;
...
void TelRilNetwork::GetImei(const AppExecFwk::InnerEvent::Pointer &response)
{
    if (cellularRadio_ != nullptr) {
        std::shared_ptr<TelRilRequest> telRilRequest = CreateTelRilRequest(HREQ_NETWORK_GET_IMEI, response);
        if (telRilRequest == nullptr) {
            TELEPHONY_LOGE("TelRilNetwork GetImei::telRilRequest is nullptr");
            return;
        }
        SendInt32Event(HREQ_NETWORK_GET_IMEI, telRilRequest->serialId_);
    }
}
...
void TelRilNetwork::NetworkRegStatusUpdated(MessageParcel &data)
{
    ...
    int32_t flagType = data.ReadInt32();
    if (observerHandler_ != nullptr) {
        TELEPHONY_LOGI("TelRilNetwork::NetworkRegStatusUpdated indicationType:%{public}d", flagType);
...
void TelRilNetwork::GetImeiResponse(MessageParcel &data)
{
    const char *buffer = data.ReadCString();
    if (buffer == nullptr) {
