...
// mnc Length type
enum MncType { UNINITIALIZED_MNC = -1, UNKNOWN_MNC = 0, DEFAULT_MNC = 2 };

enum MWIType { MSG_SIM_MWI = 0, MSG_SIM_CFI = 1, MSG_SIM_SPN = 2 };

enum StateMessage { MSG_SIM_OBTAIN_ICC_FILE_DONE = 100, MSG_ICC_REFRESH = 31, MSG_SIM_APP_READY = 1 };
...
enum MainFile { ICC_MASTER_FILE = 1, ICC_DEDICATED_FILE = 2, ICC_ELEMENTARY_FILE = 4 };
...
enum CommonConstant { INVALID_VALUE = -1, BYTE_LENGTH = 8, BYTE_VALUE = 0xFF };
...
enum SpnShowType { MSG_ID_DEFAULT = 0, SPN_CONDITION_DISPLAY_SPN = 0x01, SPN_CONDITION_DISPLAY_PLMN = 0x02 };

enum SlotRange { MIN_SLOT_INDEX = 0, MAX_SLOT_INDEX = 2 };

enum ActiveState { DEACTIVE = 0, ACTIVE = 1 };

enum CardType { ENTITY_CARD = 1, VIRTUAL_CARD = 11 };
...
enum IccDiallingNumberConstant {
    ...
    ENCODING_SHIFT_JIS = 0x05,
    ENCODING_KOREAN = 0x06,
    ENCODING_LATIN_HEBREW = 0x07,
    ENCODING_LATIN = 0x08,
    ENCODING_GSM_7BIT_ALPHABET = 0x09,
    ENCODING_GSM_DCS = 0x0A,
    ENCODING_GSM_DCS_7BIT = 0x00,
    ENCODING_GSM_DCS_8BIT = 0x01,
    ENCODING_GSM_DCS_16BIT = 0x02,
    IS91_MSG_TYPE_VOICEMAIL_STATUS = 0x82,
    IS91_MSG_TYPE_SHORT_MESSAGE_FULL = 0x83,
    IS91_MSG_TYPE_CLI = 0x84,
    IS91_MSG_TYPE_SHORT_MESSAGE = 0x85
};
