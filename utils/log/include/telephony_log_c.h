...
// Its advantage is that it is executed only once __builtin__strrchr() function
...
#define DECORATOR_HILOG(op, fmt, args...)                                           \
    do {                                                                            \
        op(LOG_CORE, "[%{public}s-(%{public}s:%{public}d)] " fmt, __FUNCTION__,     \
            GetRawFileName(__FILE__), __LINE__, ##args);                            \
    } while (0)
#endif
